CREATE     PROCEDURE dbo.SP_BUILD_DIM_DATE_INCREMENTAL
(
...
BEGIN
    ...
    -------------------------------------------------------------------
    -- 1. Generate a numbers set large enough for the date range
    -------------------------------------------------------------------
    ;WITH Numbers AS
    (
    ...
    DateSeries AS
    (
        SELECT DATEADD(DAY, N, @StartDate) AS CALENDAR_DATE
        FROM Numbers
    )
    ...
    (
        SELECT
              CAST(CONVERT(CHAR(8), DS.CALENDAR_DATE, 112) AS INT) AS SK_DATE_DIMENSION
            , DS.CALENDAR_DATE
            ...
            , CAST(GETDATE() AS TIME) AS LOAD_TIME

        FROM DateSeries DS
    ) AS S
    ON T.SK_DATE_DIMENSION = S.SK_DATE_DIMENSION
